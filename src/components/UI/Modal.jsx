...
const Modal = ({ activeID, setShowModal }) => {
  ...
  return (
    <div className="w-full h-full fixed top-0 left-0 bg-headingColor bg-opacity-40 z-10">
      <div className="max-w-[600px] absolute top-1/2 left-1/2 bg-white rounded-[8px] transform -translate-x-1/2 -translate-y-1/2 p-5">
        ...
        <div className="text-2xl text-headingColor font-[700] my-5">
          {portfolio.title}
        </div>
        ...
        <div className="mt-5 flex items-center gap-3 flex-wrap">
          ...
          {portfolio.technologies.map((item, index) => {
            return (
              <span
                key={index}
                className="bg-gray-200  py-1 px-2 text-[14px] rounded-[5px] leading-0"
              >
                {item}
              </span>
            );
